...
namespace exec {
  ...
  namespace _pool_ {
    ...
    class static_thread_pool_ {
      ...
      using bulk_sender_t = __t<bulk_sender< __id<__decay_t<Sender>>, Shape, __id<__decay_t<Fun>>>>;
      ...
      template <class Sender, class Receiver, class Shape, class Fun, bool MayThrow>
      using bulk_receiver_t = __t<
        bulk_receiver< __id<__decay_t<Sender>>, __id<__decay_t<Receiver>>, Shape, Fun, MayThrow>>;
       ...
       private:
        ...
        class sender {
         ...
         private:
          ...
          auto make_operation_(Receiver r) const -> operation_t<Receiver> {
            return operation_t<Receiver>{pool_, queue_, (Receiver&&) r, threadIndex_, constraints_};
          }

          template <receiver Receiver>
          friend auto tag_invoke(connect_t, sender s, Receiver r) -> operation_t<Receiver> {
        ...
        friend sender tag_invoke(schedule_t, const scheduler& s) noexcept {
    ...
    template <class SenderId, std::integral Shape, class FunId>
    struct static_thread_pool_::bulk_sender<SenderId, Shape, FunId>::__t {
      ...
      using completion_signatures = //
        stdexec::__try_make_completion_signatures<
          ...
          Env,
          with_error_invoke_t<Fun, __copy_cvref_t<Self, Sender>, Env>,
          __q<set_value_t>>;

      template <class Self, class Receiver>
      using bulk_op_state_t = //
        stdexec::__t<bulk_op_state< __cvref_id<Self, Sender>, __cvref_id<Receiver>, Shape, Fun>>;
    ...
    struct static_thread_pool_::bulk_shared_state {
        ...
        bulk_task(bulk_shared_state* sh_state)
          ...
          this->__execute = [](task_base* t, const std::uint32_t tid) noexcept {
            ...
            auto completion = [&](auto&... args) {
              set_value((Receiver&&) sh_state.receiver_, std::move(args)...);
            };
            ...
            if constexpr (MayThrow) {
              ...
              if (is_last_thread) {
                if (sh_state.exception_) {
                  set_error((Receiver&&) sh_state.receiver_, std::move(sh_state.exception_));
                } else {
      ...
      using variant_t = //
        __value_types_of_t< Sender, env_of_t<Receiver>, __q<__decayed_tuple>, __q<__variant>>;
      ...
      bulk_shared_state(static_thread_pool_& pool, Receiver receiver, Shape shape, Fun fn)
        : pool_{pool}
    ...
    template <class SenderId, class ReceiverId, class Shape, class Fun, bool MayThrow>
    struct static_thread_pool_::bulk_receiver<SenderId, ReceiverId, Shape, Fun, MayThrow>::__t {
      using __id = bulk_receiver;
      using receiver_concept = receiver_t;

      using shared_state = bulk_shared_state<SenderId, ReceiverId, Shape, Fun, MayThrow>;
      ...
      friend void tag_invoke(same_as<set_value_t> auto, __t&& self, As&&... as) noexcept {
        ...
        if (state.shape_) {
          ...
          state.apply([&](auto&... args) {
            set_value(std::move(state.receiver_), std::move(args)...);
          });
        }
      }
      ...
      friend auto tag_invoke(get_env_t, const __t& self) noexcept -> env_of_t<Receiver> {
        return get_env(self.shared_state_.receiver_);
      }
    };

    template <class SenderId, class ReceiverId, std::integral Shape, class Fun>
    struct static_thread_pool_::bulk_op_state<SenderId, ReceiverId, Shape, Fun>::__t {
      ...
      using bulk_rcvr = bulk_receiver_t<Sender, Receiver, Shape, Fun, may_throw>;
      using shared_state = bulk_shared_state<SenderId, ReceiverId, Shape, Fun, may_throw>;
      using inner_op_state = connect_result_t<Sender, bulk_rcvr>;
      ...
      __t(static_thread_pool_& pool, Shape shape, Fun fn, Sender&& sender, Receiver receiver)
        : shared_state_(pool, (Receiver&&) receiver, shape, fn)
        , inner_op_{connect((Sender&&) sender, bulk_rcvr{shared_state_})} {
      }
    };
    ...
    namespace schedule_all_ {
      ...
      struct operation_base_with_receiver : operation_base<Range> {
        ...
        operation_base_with_receiver(Range range, static_thread_pool_& pool, Receiver&& receiver)
      ...
      struct next_receiver {
        ...
        struct __t {
          ...
          friend void tag_invoke(SetValue, Self&& self) noexcept {
            ...
            if (countdown == 1) {
              set_value((Receiver&&) self.op_->receiver_);
            }
          }
          ...
          friend auto tag_invoke(GetEnv, Self&& self) noexcept -> env_of_t<Receiver> {
            return get_env(self.op_->receiver_);
          }
        };
      };
        ...
        class __t : operation_base_with_receiver<Range, Receiver> {
          ...
          friend void tag_invoke(start_t, Self& op) noexcept {
            ...
            for (std::size_t i = i0; i < size; ++i) {
              op.items_[i].__construct_with([&] {
                return connect(set_next(op.receiver_, ItemSender{&op, it + i}), NextReceiver{&op});
              });
              start(op.items_[i].__get());
            }
            std::unique_lock lock{op.start_mutex_};
